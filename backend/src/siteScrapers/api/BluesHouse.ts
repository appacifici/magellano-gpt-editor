...
class BluesHouse extends BaseApi {
    action:string;
    ...
    async init() {
        switch (this.action) {
            case 'readSitemap':                
                await this.readSimpleSitemap('blueshouse.it', this.scrapeWebsite);           
    ...
    private async scrapeWebsite(url: string): Promise<ScrapedData | null> {
        try {
            ...
            await writeErrorLog(`scrapeWebsite: BluesHouse: Errore durante lo scraping della pagina`);
            await writeErrorLog(error);
            console.error('scrapeWebsite: BluesHouse: Errore durante lo scraping della pagina:', error);
            return null;
        }
    }
}

export default BluesHouse;